...
class MetricLogger:
    def __init__(
        ...
        self._tensorboard_writer = SummaryWriter(log_dir=out_dir)
    ...
    def log_metrics(self, val: bool = False, **kwargs) -> None:
        for metric_name, metric_val in kwargs.items():
            ...
            if not val:
                self._tensorboard_writer.add_scalar(
                    f"Train/{metric_name}", metric_val, self._step_counter + 1
                )
            else:
                self._tensorboard_writer.add_scalar(
                    f"Valid/{metric_name}", metric_val, self._step_counter + 1
                )
        ...
        if not val:
            ...
            if self._step_counter % self._window_size == 0:
                if not self._quiet:
                    mean_metric_str = self.__format_metric_dict(
                        self._metrics, self._step_counter
                    )
                ...
                if self._aml_run is not None:
                    for metric_name, metric_val in self._windowed_metrics.items():
                        self._aml_run.log(
                            f"{self._metric_name_prefix}{metric_name}",
                            metric_val / self._window_size,
                        )
